...
def showData():
    try:
        cnx = mysql.connector.connect(user=creds.user, password=creds.password,
                                      host=creds.host, database=creds.database, auth_plugin=creds.auth_plugin)
        Cursor = cnx.cursor()
...
def insertMember():
    try:
        cnx = mysql.connector.connect(
            user=creds.user, password=creds.password, host=creds.host, database=creds.database, auth_plugin=creds.auth_plugin)
        Cursor = cnx.cursor()
...
def UpdateMember():
    try:
        ...
        Cursor = cnx.cursor()
        mno = input(
            "Enter Member Code of Member to be Updated from the Library : ")
        query = ("SELECT * FROM member WHERE mno = %s")
        rec_srch = (mno,)
